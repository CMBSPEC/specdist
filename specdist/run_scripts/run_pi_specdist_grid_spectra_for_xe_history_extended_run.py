...
photon_injection_case = 'extended_run_xe_history_120820'
compute_no_injection_results = 'yes'
...
# if we want to set finj to a value corresponding to the constraints:
if finj_from_fisher == 'yes' and compute_no_injection_results != 'yes':
    firas = sd.firas()
    edges = sd.edges()
    a_dict = {}
    a_dict['firas'] = firas
...
gi_array = np.logspace(-8,-20,80)

if compute_no_injection_results == 'yes':
    xmin = min(xi_array)
    Gamma_max = max(gi_array)
    ct.save_dir_name = photon_injection_case + '_no_injection'
    ct.ct_Gamma_dec = Gamma_max
    p_array = [xmin]
    args['param_values_array'] = p_array
    R = ct.compute_no_injection_spectrum_and_Xe_history(**args)
else:
    for Gamma_X in gi_array[0:40]:
