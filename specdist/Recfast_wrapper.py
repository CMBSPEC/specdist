...
class recfast:
    ...
    def create_tmp_dir_to_store_full_rf_outputs(self,rm = 'yes'):
        ...
        subprocess.call(['mkdir','-p',self.path_to_rf_param_file+self.tmp_dir_name])
    ...
    def run_recfast_not_parallel(self,id_gamma,**args):
        # if id_gamma == 0:
        self.create_tmp_dir_to_store_full_rf_outputs(rm='no')
        ...
        if args['multi_params'] == 'no':
            ...
            # else:
            #     array_args = args['param_values_array']
            print('no multi params: ', args['param_values_array'])
            p_val = args['param_values_array'][0]
            new_ar = np.zeros((id_p+1))
            new_ar[id_p] = p_val
            array_args = new_ar
            # print(array_args)
            ...
            results = [self.compute_recfast_parallel(id_p,param_values_array=array_args,param_name=args['param_name'])]
            ...
            # array_args = args['param_values_array']
            print('with multi params: ', args['param_values_array'])
            p_val_1 = args['param_values_array'][0][0]
            p_val_2 = args['param_values_array'][1][0]
            new_ar_1 = np.zeros((id_p+1))
            new_ar_2 = np.zeros((id_p+1))
            new_ar_1[id_p] = p_val_1
            new_ar_2[id_p] = p_val_2
            array_args = (new_ar_1,new_ar_2)
            # array_args = new_ar
            ...
            results = [self.compute_recfast_parallel_multi_params(id_p,param_values_array=array_args,param_name=args['param_name'])]
        # pool.close()
        #self.clear()
    ...
    # here we don't assume a linear scaling but solve the equation for fdm exactly:
    def pi_run_pca_constraints_with_recfast_exact(self,xinj_values,gammai_array,id_gamma,fdmi_array,N_iter = 1,*args,**kwargs):
        PCA_eigen_modes = kwargs['PCA_modes']
        recfast = kwargs['recfast']
        f_dm_pca = {}
        f_dm_pca['curves'] = []
        ...
        for id_iter in range(N_iter):
            ...
            print('initial fdms (not parallel) = ', fdmi_array)
            # in fact we do only one xinj value (TBD: remove the loop)
            for xinj_asked in xinj_values:
                xdec = xinj_asked
                curves = {}
                curves['Gamma_inj'] = gammai_array
                ...
                self.rf_xinj0 = xdec
                self.rf_f_dec = fdmi_array[0] # this is the first guess
                ...
                # args['id_params'] = id_gamma
                #rf.save_dir_name = 'case_' + '_xdec_' + str_dir
                R = self.run_recfast_not_parallel(id_gamma,**args)
                ...
                if id_iter == 0:
                    ...
                    self.rf_f_dec = 1.e-300
                    #self.save_dir_name = 'case_no_inj' + '_xdec_' + str_dir
                    R_no_inj = self.run_recfast_not_parallel(id_gamma,**args)
        ...
        return fdm_pca_array

    def solve_pca_exact(self,id_gamma,gamma_array,xdec,**a_dict):
        print('solving pca exact')
        gamma = gamma_array[id_gamma]
        def f(log10x,gammav,xdecv):
            x = 10**log10x
            fdmi_array = [x]
            N_iter = 1
            gi_array = [gammav]
            xdec = [xdecv]
            return self.pi_run_pca_constraints_with_recfast_exact(xdec,gi_array,id_gamma,fdmi_array,N_iter,**a_dict)[0]-1.
        try:
            root = optimize.brentq(f,-20, 0,args = (gamma,xdec))
            result = 10**root
        except ValueError:
            result = np.nan
        return result
